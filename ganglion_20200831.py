...
class Ganglion:
	...
	def __call__(self, input_data): #, w_mode=True):
		...
		self.potentials[self.index_list[0][1]] += np.dot(input_data[self.index_list[0][0]], (self.synapse_weight_list[0]))
		
		for idx, syn_matrix in enumerate(self.synapse_weight_list[1:-1]):
			self.potentials[self.index_list[idx+1][1]] += np.dot(self.__activation_function(self.potentials[self.index_list[idx+1][0]]), syn_matrix)
		
		out = self.__activation_function(np.dot(self.__activation_function(self.potentials[self.index_list[-1][0]]), self.synapse_weight_list[-1]))
	...
	def _build_weights(self):
		...
		for syn_type_matrix in self.synapse_type_list:
			syn_weight_init = np.random.randint(-255, 256, size=syn_type_matrix.shape).astype(np.float32) #np.random.uniform(0.01, 5.0, (syn_type_matrix.shape)).astype(np.float32)
			self.synapse_weight_list.append(syn_weight_init)

	def __builder(self, rows: int, columns: int):
		return np.random.randint(0, 2, size=(rows, columns)).astype(np.float32)
	...
	def __activation_function(self, s_vector):
		return 1 / (1 + np.exp(-(s_vector * 0.02))) #-10.0))
	...
	def __potential_limiter(self):
		self.potentials[self.potentials > 500.0] = 500.0
		self.potentials[self.potentials < -50.0] = -50.0