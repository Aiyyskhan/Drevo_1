...
def __mutate(ind_arr: np.ndarray, syn_weight, max_percent: int) -> np.ndarray:
	...
	if syn_weight:
		ind_arr_rav[mixed_index_slice] = ind_arr_rav[mixed_index_slice] + np.random.randint(-30, 30, num_mixed_indices).astype(np.float32) #np.random.uniform(-1.0, 1.0, num_mixed_indices).astype(np.float32) 
	else:
